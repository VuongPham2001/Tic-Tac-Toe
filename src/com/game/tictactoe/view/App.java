...
import com.game.tictactoe.bean.RoundedBorder;
import com.game.tictactoe.bean.Setting;
import com.game.tictactoe.bo.TicTacToeAI;
import com.game.tictactoe.dao.SettingDao;
import com.game.tictactoe.values.Value;
...
public class App extends JFrame implements MouseListener {
    ...
    public void newGame() {
        ...
        tictactoe = new TicTacToeAI(setting.getMode());
        ...
        if (setting.getMode() == 1) {
            ...
            int x = tictactoe.getNextX();
            int y = tictactoe.getNextY();
    ...
    public App() {
        ...
        setting = SettingDao.LoadSettingInfo();
        tictactoe = new TicTacToeAI(setting.getMode()); // khởi tạo AI
        ...
        if (setting.getMode() == 1)
            updateTableCells(tictactoe.getNextX(), tictactoe.getNextX(), Value.AI_VALUE);
        ...
        JLabel lbltitle = new JLabel("TICTACTOE");
    ...
    public void mouseClicked(MouseEvent element) {
        ...
        if (element.getClickCount() == 1) { // người dùng click dạo(click 1 lần)
            ...
            if (tictactoe.isClickable(x, y)) {// nếu ô này chưa được đánh
                tictactoe.update(x, y, Value.USER_VALUE); // update matrix
                System.out.println("\n----------------------------------------------------------------------");
                ...
                x = tictactoe.getNextX();
                y = tictactoe.getNextY();
                updateTableCells(x, y, Value.AI_VALUE);
    ...
    private boolean checkResult(int player) {
        if (player == Value.USER_VALUE) {
            boolean result = tictactoe.checkWinner(Value.USER_VALUE);
            if (result == true) {
            ...
            boolean result = tictactoe.checkWinner(Value.AI_VALUE);
            if (result == true) {
